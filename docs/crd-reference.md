...
Field-by-field reference for every CRD in `cloudflare.io/v1alpha1`. For end-to-end walkthroughs and topical docs, see [`docs/README.md`](README.md).
...
Manages a Cloudflare Tunnel lifecycle, auto-generates the `cloudflared` credentials Secret, and (optionally) reconciles the operator-managed `cloudflared` Deployment + ConfigMap + ServiceAccount that keep the tunnel connected.

For the full operator-managed connector story — connector spec fields in depth, ingress aggregation, hand-authored `CloudflareTunnelRule`, migration from self-managed cloudflared — see [`tunnels.md`](tunnels.md). This section is the field reference.
...
| `nodeSelector` / `tolerations` / `affinity` / `topologySpreadConstraints` | | | Standard pod placement controls. |

### `routing` sub-fields

| Field | Type | Description |
|-------|------|-------------|
| `defaultBackend.url` | string | Raw backend URL. Mutually exclusive with `defaultBackend.serviceRef`. |
...
- **Credential generation**: Automatically creates a Kubernetes Secret containing `credentials.json` with the tunnel token. The credentials path is also written into the rendered `config.yaml` as `credentials-file:`, so cloudflared (operator-managed or self-managed) can resolve it from the config alone.
- **Tunnel identity in `config.yaml`**: When the connector is enabled, the rendered `config.yaml` carries `tunnel: <tunnelID>` and `credentials-file: …` ahead of the `ingress:` block. Container Args are minimal: `[tunnel, --config, /etc/cloudflared/config.yaml, run]`.
